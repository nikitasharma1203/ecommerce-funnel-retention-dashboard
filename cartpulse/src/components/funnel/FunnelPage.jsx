...
export default function FunnelPage() {
  const { data } = useData();
  const { funnelSteps, dataSource } = data;
  ...
  }, [funnelSteps]); // re-animate when data changes
  ...
  const DCOLORS = { Mobile:"#ef4444", Desktop:"#10b981", Tablet:"#f59e0b" };
  const SCOLORS = ["#6366f1","#10b981","#f59e0b","#06b6d4","#8b5cf6"];

  // compute step-to-step CVRs from live data
  const stepCVRs = funnelSteps.map((step, i) =>
    i === 0 ? null : `${((step.count / funnelSteps[i-1].count)*100).toFixed(1)}%`
  );
  ...
  return (
    <div>
      ...
      {/* Step-to-step CVR cards */}
      <div className="kpi-grid">
        {funnelSteps.slice(1).map((step, i) => (
          <div key={step.name} className="kpi-card">
            <span style={{ position:"absolute", top:14, right:14, fontSize:18, opacity:0.5 }}>
              {["👁","🛒","💳","✅"][i]}
            </span>
            <div style={{ fontSize:11, fontWeight:600, color:"var(--text3)", textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:8 }}>
              {funnelSteps[i].name} → {step.name}
            </div>
      ...
      <div className="section">
        <div className="section-header">
          ...
          <span className="section-badge">{dataSource === "csv" ? "CSV Data" : "All Devices · All Sources"}</span>
        </div>
        ...
        <div style={{ padding:"16px 0" }}>
          {funnelSteps.map((step, i) => (
            <div key={step.name}>
              {i > 0 && (
                <div style={{ textAlign:"center", color:"var(--text4)", fontSize:11, margin:"3px 0 3px 124px" }}>
                  ▼ Drop-off:{" "}
                  ...
                  <span style={{ color:"var(--text3)" }}>
                    {((funnelSteps[i-1].count - step.count)/1000).toFixed(1)}K users lost
                  </span>
                </div>
              )}
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dropByDevice} margin={{ top:4, right:10, left:-10, bottom:0 }}>
                ...
                <Bar dataKey="abandon" radius={[6,6,0,0]} name="Abandon %">
                  {dropByDevice.map(d => <Cell key={d.device} fill={DCOLORS[d.device]||"#6366f1"} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div style={{ height:220 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dropBySource} margin={{ top:4, right:10, left:-10, bottom:0 }}>
                ...
                <Tooltip content={<TT />} />
                <Bar dataKey="abandon" radius={[6,6,0,0]} name="Abandon %">
                  {dropBySource.map((_,i) => <Cell key={i} fill={SCOLORS[i%SCOLORS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
