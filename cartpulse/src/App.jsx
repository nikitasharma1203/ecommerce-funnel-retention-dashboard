...
function Dashboard() {
  ...
  const { user } = useAuth();
  const { data, loadUserData } = useData();

  // Restore persisted state on login
  useEffect(() => {
    if (user && !user.isDemo) loadUserData(user);
  }, [user, loadUserData]);
  ...
  return (
    <div className="app-shell">
      <Topbar
        ...
        onDownloadPDF={() => generatePDFReport(null, data.aiInsights, user?.displayName || "User")}
      />
...
function AppInner() {
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#09111e", flexDirection:"column", gap:16 }}>
